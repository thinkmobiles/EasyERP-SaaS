...
module.exports = function (app, mainDb) {
    ...
    var fs = require("fs");
    var SaasHandler = require('../handlers/saas');
    var saas = new SaasHandler(mainDb);
    ...
    app.post('/account', saas.accountLoad);
    ...
    app.get('/logout', function (req, res, next) {
        ...
        if (req.session) {
            if(mainDb.dbsObject && req.session.lastDb){
                account = req.session.lastDb;

                mainDb.dbsObject[req.session.lastDb].close(function(){
                    delete mainDb.dbsObject[req.session.lastDb];
                    console.log('===== disconnected from db ' + req.session.lastDb);
        ...
        tracker.track({
            ...
            subDomainName: account ? account : subdomainParser(req).accountName
        });
...
//---------------------Opportunities---------------------
    ...
    function notFound (req, res, next) {
        ...
        /*tracker.track({
            ...
            city: (geo) ? geo.city : '',
            region: geo ? geo.region : '',
            subDomainName: subdomainParser(req).accountName
        });*/
